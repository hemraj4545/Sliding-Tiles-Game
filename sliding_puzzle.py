...
def main(argv):
    ...
    while True:
        try:
            l = []
            m = []
            with open('sliding.json') as f:
                data = json.load(f)
            init_state =  data['start']
            for i in range(len(init_state)):
                for j in range(len(init_state)):
                    init_state[i][j] = str(init_state[i][j])
            ...
            for i in range(len(goal_state)):
                for j in range(len(goal_state)):
                    ...
                    goal_state[i][j] = int(goal_state[i][j])
                    m.append(goal_state)
                    
            if len(l) == len(m) and len(m) == n * n:
                break
            else:
